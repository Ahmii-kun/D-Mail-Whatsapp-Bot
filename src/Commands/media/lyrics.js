...
export default class LYRICS extends BaseCommand {
    ...
    async run(M, { context }) {
        ...
        const data = await this.client.utils.fetch(`${this.client.config.API_URL}genius?query=${term}`)
        ...
        const lyrics = await this.client.utils.fetch(`${this.client.config.API_URL}lyrics?url=${data[0].url}`)
        text += `\n\n${lyrics}`
